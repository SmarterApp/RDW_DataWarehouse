...
def generate_data_from_config_file(config_module, output_dict):
    ...
    for state in states:
        ...
        state_level_staff = generate_non_teaching_staff(number_of_state_level_staff, config_module, state_code=current_state.state_code)
        ...
        for district_type_name in districts_by_type.keys():
            ...
            for district in districts:
                ...
                district_level_staff = generate_non_teaching_staff(number_of_district_level_staff, config_module, state_code=current_state.state_code,
...
def populate_school(institution_hierarchy, school_type, assessments, subject_percentages, config_module, output_dict):
    ...
    school_level_staff = generate_non_teaching_staff(number_of_school_level_staff, config_module, state_code=institution_hierarchy.state_code,
    ...
    for grade in grades:
        ...
        name_list_dictionary = generate_name_list_dictionary(NAMES_TO_PATH_DICT)
        students_in_grade = generate_students_from_institution_hierarchy(number_of_students_in_grade, institution_hierarchy, grade, -1, name_list_dictionary[BIRDS], config_module)
        for subject_name in constants.SUBJECTS:
            ...
            for section in sections_in_grade:
                ...
                teachers_in_section = generate_teaching_staff_from_institution_hierarchy(number_of_staff_in_section, institution_hierarchy, section.section_guid, config_module)
...
def generate_institution_hierarchy_from_helper_entities(state, district, school, config_module):
    '''
...
def generate_students_from_institution_hierarchy(number_of_students, institution_hierarchy, grade, section_guid, street_names, config_module):
    '''
...
def generate_teaching_staff_from_institution_hierarchy(number_of_staff, institution_hierarchy, section_guid, config_module):
    '''
...
def generate_non_teaching_staff(number_of_staff, config_module, state_code='NA', district_guid='NA', school_guid='NA'):
    '''
...
def get_flat_grades_list(school_config):
    ...
    for school_type in school_config:
        grades.extend(school_config[school_type][constants.GRADES])
