...
if __name__ == '__main__':
    ...
    print("Total number of districts in the state -- %i" % len(state_pop.districts))
    print_devider()

    # print out school information
    sch_num = 0
    for i in range(len(state_pop.districts)):
        print('Number of schools in district %i --  %i' % (i + 1, len(state_pop.districts[i].schools)))
        sch_num += len(state_pop.districts[i].schools)
    print_devider()
    print("Total number of schools in the state -- %i" % sch_num)
    print_devider()

    # initialize a Demographics with a demographicStats.csv
    dem_obj = demographics.Demographics(DEMO_FILE)
    state_pop.get_state_demographics(dem_obj, 'typical1')
    demo_totals = state_pop.state_demographic_totals
