...
class SAMLResposneSignatureVerification():
    ...
    def verify_signature(self):
        verify = False
        if self.cert_pem_filename is not None and self.path is not None:
            cmd = ['xmlsec1', '--verify', '--id-attr:ID', 'urn:oasis:names:tc:SAML:2.0:assertion:Assertion', '--trusted-pem', self.cert_pem_filename, self.path]
            try:
