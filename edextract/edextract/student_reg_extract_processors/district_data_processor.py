...
class DistrictDataProcessor(EdOrgDataProcessor):
    ...
    def process_yearly_data(self, data_row):
        self._call_academic_year_trackers(data_row[AttributeFieldConstants.DISTRICT_ID], data_row)
        self._add_to_edorg_hierarchy(data_row[AttributeFieldConstants.DISTRICT_ID],
    ...
    def process_matched_ids_data(self, data_row):
        if self._is_matched_district(data_row):
            self._call_matched_ids_trackers(data_row[AttributeFieldConstants.DISTRICT_ID], data_row)

    def process_asmt_outcome_data(self, data_row):
        self._call_asmt_trackers(data_row[AttributeFieldConstants.DISTRICT_ID], data_row)

    def _is_matched_district(self, data_row):
        return data_row[AttributeFieldConstants.DISTRICT_ID] == data_row[AttributeFieldConstants.PREV_DISTRICT_ID]
