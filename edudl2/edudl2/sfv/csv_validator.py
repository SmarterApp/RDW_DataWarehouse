    ...
    def execute(self, dir_path, file_name, batch_sid):
        ...
        try:
            ...
            if not self.are_eq(header_row, self.expected_csv_fields):
                ...
                new_expected = [x for x in self.expected_csv_fields if x not in {'op', 'completestatus', 'administrationcondition'}]
                new_header_row = [x for x in header_row if x not in {'op', 'completestatus', 'administrationcondition'}]
