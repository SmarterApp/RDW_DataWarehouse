...
class TestJsonUtils(unittest.TestCase):
    ...
    def test_get_from_valid_json(self):
        ...
        value = get_key_from_json(self.test_expanded_dir, 'content')
    ...
    def test_no_json(self):
        self.assertRaises(IOError, get_key_from_json, self.test_expanded_dir, 'content')

    def test_get_nested_keys(self):
        shutil.copy(os.path.join(self.data_dir, 'test_valid_content_type.json'), self.test_expanded_dir)
        key = get_key_from_json(self.test_expanded_dir, 'source', 'testregcallbackurl')
        self.assertEqual('StateTestReg.gov/StuReg/CallBack'.lower(), key)

    def test_missing_nested_keys(self):
        shutil.copy(os.path.join(self.data_dir, 'test_valid_content_type.json'), self.test_expanded_dir)
        value = get_key_from_json(self.test_expanded_dir, 'source', 'incorrectkey1', 'incorrectkey2')
        self.assertEqual(None, value)

    def test_case_insensitivity(self):
        shutil.copy(os.path.join(self.data_dir, 'test_valid_content_type.json'), self.test_expanded_dir)
        value_from_lower = get_key_from_json(self.test_expanded_dir, 'source', 'testregcallbackurl')
        value_from_upper = get_key_from_json(self.test_expanded_dir, 'Source', 'TestRegCallbackURL')

        self.assertEqual(value_from_lower, value_from_upper)
