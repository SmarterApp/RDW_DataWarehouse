...
class Test_Error_In_Migration(unittest.TestCase):
    ...
    def validate_edware_database(self, schema_name):
        with get_target_connection(self.tenant, schema_name) as ed_connector:
            ...
            prod_output_data = select([fact_table_vw.c.rec_status]).where(fact_table_vw.c.student_guid == '3efe8485-9c16-4381-ab78-692353104cce', )
            ...
            prod_data = select([fact_table.c.rec_status]).where(fact_table.c.student_guid == '3efe8485-9c16-4381-ab78-692353104cce', )
    ...
    def validate_prod(self):
        with get_prod_connection(self.tenant) as conn:
            ...
            query = select([fact_table], and_(fact_table.c.student_guid == '3efe8485-9c16-4381-ab78-692353104cce', fact_table.c.rec_status == 'D'))
            result = conn.execute(query).fetchall()
            expected_no_rows = 1
            ...
            fact_asmt_pr = select([fact_table_pr], and_(fact_table_pr.c.student_guid == '3efe8485-9c16-4381-ab78-692353104cce', fact_table_pr.c.rec_status == 'D'))
