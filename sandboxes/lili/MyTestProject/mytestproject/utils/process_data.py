from mytestproject.helpers import enums
from mytestproject.helpers.enums import Scope, Grain, SchoolGroupType, GradeDividers
...
def get_param_info(parameters):
    ...
    grade_dividers_on = GradeDividers.get_by_code(str(parameters['grade_divider']))['boolean']
...
def build_scope_group(param_school_group_type, row, school_group_type, scope_info):        
    current_scope_group = {
        'grade_groups': [],
        'school_group_type': {
            'code': param_school_group_type,
            'name': school_group_type['name'] if param_school_group_type != enums.NOT_APPLICABLE_CODE else None
         },
...
def build_bar_segment(row):
    bar_segment = {
            'performance_level': None if row['perf_level_code'] is None else 
            {
                'code': row['perf_level_code'],
                'name': row['perf_level_name']
            },
