...
function create_sym_link_for_apache {
    ...
    ls -l ${WORKSPACE}/services/config/linux/opt/edware/conf/celeryd.conf
    echo sed 's/"celery"/"jenkins"/' ${WORKSPACE}/services/config/linux/opt/edware/conf/celeryd.conf  ${CELERY_CONF}
...
function build_rpm {
    ...
    scp /home/jenkins/rpmbuild/RPMS/x86_64/smarter${SMARTER_ENV_NAME}-${RPM_VERSION}-${BUILD_NUMBER}.el6.x86_64.rpm pynest@${PYNEST_SERVER}:/opt/wgen/rpms
