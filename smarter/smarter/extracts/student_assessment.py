...
def get_extract_assessment_query_iab(params):
    ...
    with EdCoreDBConnection(state_code=state_code) as connector:
        ...
        query = select_with_context([
            ...
            fact_block_asmt_outcome.c.administration_condition.label(fact_block_asmt_outcome_label.get('administration_condition', 'administration_condition')),
            case([
                (fact_block_asmt_outcome.c.complete.is_(True), 'Complete'),
                (fact_block_asmt_outcome.c.complete.is_(False), 'Partial')
            ], else_=None).label(fact_asmt_outcome_vw_label.get('complete', 'complete'))
        ], from_obj=[
            fact_block_asmt_outcome
            .join(dim_student, and_(fact_block_asmt_outcome.c.student_rec_id == dim_student.c.student_rec_id))
            .join(dim_asmt, and_(dim_asmt.c.asmt_rec_id == fact_block_asmt_outcome.c.asmt_rec_id, dim_asmt.c.asmt_type == asmt_type))
            .join(dim_inst_hier, and_(dim_inst_hier.c.inst_hier_rec_id == fact_block_asmt_outcome.c.inst_hier_rec_id))
        ], permission=RolesConstants.SAR_EXTRACTS, state_code=state_code)
