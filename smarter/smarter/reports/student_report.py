...
def __prepare_query(connector, student_id, assessment_id):
    ...
    query = select([fact_asmt_outcome.c.student_id,
        ...
        from_obj=[
            fact_asmt_outcome.join(dim_student, fact_asmt_outcome.c.student_id == dim_student.c.student_id).join(dim_teacher, fact_asmt_outcome.c.teacher_id == dim_teacher.c.teacher_id).join(dim_asmt, dim_asmt.c.asmt_id == fact_asmt_outcome.c.asmt_id)])
