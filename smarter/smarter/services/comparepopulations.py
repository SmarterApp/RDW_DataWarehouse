...
from smarter.utils.database_connections import getDatabaseConnection, getSQLAlchemyConnection
from postgresql.exceptions import Exception

_supported_keys = ("segment_by", "grades", "year_range", "time_period",
                   "teacher_filter", "district_filter", "school_filter",
...
def generateComparePopulationsReportAlchemy(parameters):
    if isinstance(parameters, str):
        try:
            # convert string input to dictionary
            parameters = eval(parameters.strip())
