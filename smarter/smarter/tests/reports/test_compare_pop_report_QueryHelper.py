...
class Test(Unittest_with_smarter_sqlite_no_data_load):
    ...
    def test_build_columns_state_view(self):
        with SmarterDBConnection() as connection:
            helper = QueryHelper(connection, ParameterManager(Parameters(get_param_state_view())))
        ...
        self.assertEqual(columns[1].name, Constants.DISTRICT_ID, 'test for alias name')
    ...
    def test_build_columns_district_view(self):
        with SmarterDBConnection() as connection:
            helper = QueryHelper(connection, ParameterManager(Parameters(get_param_district_view())))
            columns = helper.build_columns()
            dim_inst_hier = connection.get_table(Constants.DIM_INST_HIER)
            dim_asmt = connection.get_table(Constants.DIM_ASMT)
        ...
        self.assertEqual(columns[1].name, Constants.SCHOOL_ID, 'test for alias name')
        self.assertEqual(columns[1].element.table.name, dim_inst_hier.name)
    ...
    def test_build_columns_school_view(self):
        with SmarterDBConnection() as connection:
            helper = QueryHelper(connection, ParameterManager(Parameters(get_param_school_view())))
            columns = helper.build_columns()
            dim_asmt = connection.get_table(Constants.DIM_ASMT)
