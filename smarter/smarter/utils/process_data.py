...
def get_param_info(parameters):
    ...
    if 'segment_by' in parameters.keys():
        ...
        if grain_info is not None and grain_info['inst_hierarchy_order'] is None:
            grain_info = None
    ...
    if grain_info is not None and scope_info is not None and grain_info['inst_hierarchy_order'] is not None and grain_info['inst_hierarchy_order'] >= scope_info['inst_hierarchy_order']:
...
def build_bar_group(grain_info, row):
    current_bar_group = {
        'bars': [],
        'state_group': None if grain_info['inst_hierarchy_order'] is not None and grain_info['inst_hierarchy_order'] > Grain.GroupOfState['inst_hierarchy_order'] else {
            'code': row['state_group_code'],
            'name': row['state_group_name']
        },
        'state': None if grain_info['inst_hierarchy_order'] is not None and grain_info['inst_hierarchy_order'] > Grain.State['inst_hierarchy_order'] else {
            'code': row['state_code'],
            'name': row['state_name']
        },
        'school_group': None if grain_info['inst_hierarchy_order'] is not None and grain_info['inst_hierarchy_order'] > Grain.District['inst_hierarchy_order'] else {
            'code': row['school_group_code'],
            'name': row['school_group_name']
        },
        'school': None if grain_info['inst_hierarchy_order'] is not None and grain_info['inst_hierarchy_order'] > Grain.School['inst_hierarchy_order'] else {
            'code': row['school_code'],
            'name': row['school_name']
        },
        'teacher': None if grain_info['inst_hierarchy_order'] is not None and grain_info['inst_hierarchy_order'] > Grain.Teacher['inst_hierarchy_order'] else {
            'code': row['teacher_code'],
            'name': row['teacher_name']
        },
        'student': None if grain_info['inst_hierarchy_order'] is not None and grain_info['inst_hierarchy_order'] > Grain.Student['inst_hierarchy_order'] else {
