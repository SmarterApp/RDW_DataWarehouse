...
select 'Grade'     as report_comparison_type
     ...
     , -- FIRST_VALUE(school_name)
        school_name over (partition by school_loc_code order by CAST(academic_year_code AS integer) desc)
        as school_name
        ...
        select
         ...
         , di.account_sid as account_abbrev
             , case when min(di.state_sid)
               ...
               <> max(di.state_sid)
               over(partition by di.account_sid)
               then 'Multiple States' else di.state_name end as state_name
             , case when min(di.state_sid)
               over(partition by di.account_sid)
               <> max(di.state_sid)
               over(partition by di.account_sid)
               then 'Multi States' else di.state_code end as state_abbrev
             , case when min(di.state_sid)
               over(partition by di.account_sid)
               <> max(di.state_sid)
               over(partition by di.account_sid)
               then '-1' else di.state_sid end as state_code
