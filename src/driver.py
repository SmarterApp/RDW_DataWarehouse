...
def start_pipeline(file_path):
    ...
    udl2.W_file_splitter.task.apply_async([{'input_file':landing_zone_file_path}], queue='Q_files_to_be_split')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', dest='row_multiplier', type=int, default=1, help="number of times to muliply the rows in the seed file.")
    parser.add_argument('-c', dest='column_multiplier', type=int, default=1, help="number of times to multiply the columns in the seed file.")
    ...
    parser.add_argument('-m', dest='output_metadata_csv', required=False, default=os.path.join(DATAFILES, 'output_meta_data.csv'), help="path to the file that will contain the newly generated csv metadata.")
