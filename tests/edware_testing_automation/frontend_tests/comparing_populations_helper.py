...
class ComparingPopulationsHelper(SessionShareHelper):
    ...
    def check_cpop_report_info(self):
        ...
        try:
            WebDriverWait(self.driver, 25).until(
                    expected_conditions.visibility_of_element_located((By.CLASS_NAME, "reportInfoPopover")))
        except:
    ...
    def check_grid_titles(self, column_index, column_title):
        ...
        '''
        grid_headers = self.driver.find_element_by_class_name("jqg-second-row-header").find_elements_by_tag_name("th")
    ...
    def check_grid_sub_headers(self, column_index, column_title):
        ...
        :type column_title: string
        '''
        grid_headers = self.driver.find_element_by_class_name("ui-jqgrid-labels").find_elements_by_tag_name("th")
        self.assertEqual(column_title, str(grid_headers[column_index].text)), "Grid column header not found"
    ...
    def check_list_of_population(self, list_id, link_text, math_progress_bar, math_total_students, ela_progress_bar,
        ...
        :type ela_total_students: string
        '''
        grid_values = self.driver.find_element_by_class_name("ui-jqgrid-bdiv").find_element_by_id(
    ...
    def check_progress_bar_tooltip_is_found(self, section, expected_legend):
        ...
        :type section: string
        '''
        element_to_click = self.driver.find_element_by_class_name("ui-jqgrid-ftable").find_element_by_class_name(
                "progress").find_element_by_class_name("bar")
        hover_mouse = ActionChains(self.driver).move_to_element(element_to_click)
        hover_mouse.perform()
        time.sleep(3)
        try:
            WebDriverWait(self.driver, 15).until(lambda driver: driver.find_element_by_class_name("popover"))
        except:
            self.driver.save_screenshot('/tmp/screenshot_tooltip.png')
            self.assertTrue(False, "Error in viewing the tooltip")
        popover_legend = self.driver.find_element_by_class_name("popover-content").find_elements_by_tag_name("li")
        ...
        temp = self.driver.find_element_by_class_name("ui-jqgrid-ftable").find_element_by_class_name("summaryTitle")
        hover_mouse_out = ActionChains(self.driver).move_to_element(temp)
    ...
    def sort_by_asmt(self, asmt, expected_top_row, expected_header):
        ...
        if asmt == 'Math':
            ...
            index = 3
        columns = self.driver.find_elements_by_class_name("ui-th-column")
        columns[index].find_element_by_class_name("s-ico").click()
        self.check_topmost_row(expected_top_row)
        self.driver.save_screenshot('/tmp/nidhi_sort_temp.png')
        new_columns = self.driver.find_elements_by_class_name("ui-th-column")
    ...
    #        :type expected_top_row: string
    #        '''
    #        grid_rows = self.driver.find_element_by_class_name("ui-jqgrid-bdiv").find_element_by_id("gridTable").find_elements_by_tag_name("tr")
    ...
    def sort_by_entity_name(self, sorted_value):
        ...
        '''
        self.driver.find_element_by_id("jqgh_gridTable_name").find_element_by_class_name("s-ico").click()
    ...
    def check_alignment(self, math_cumulutive_perc, ela_cumulative_perc):
        ...
        '''
        self.assertEqual("Align:", str(self.driver.find_element_by_class_name("alignLabel").text),
                         "Align Label not displayed correctly.")
        # anchor = self.driver.find_element_by_id("actionBar").find_element_by_class_name("alignmentItem").find_element_by_class_name("align_button_area").find_element_by_tag_name("a")
        checkboxes = self.driver.find_elements_by_class_name('alignmentCheckbox')
        checkboxes[0].click()
    ...
    def check_bars_aligned(self):
        ...
        asmt_columns = self.driver.find_element_by_class_name("ui-jqgrid-ftable")
    ...
    def check_topmost_row(self, expected_top_row):
        ...
        :type expected_top_row: string
        '''
        all_rows = self.driver.find_element_by_class_name("ui-jqgrid-bdiv").find_element_by_id(
